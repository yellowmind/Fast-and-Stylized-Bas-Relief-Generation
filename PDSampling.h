...
class PenroseSamplerW : public PDSampler {
	int width; 
	int height;
	std::vector<unsigned int> imap;

public:
	PenroseSamplerW(float radius, int _width, int _height, std::vector<unsigned int> &_imap) : 
		PDSampler(radius, false, false), width(_width), height(_height), imap(_imap) {};
