...
void StreamLine::GenStreamLine(int i, int j, int L){
	origin = Center(i, j);
	//getVF(b);
...
void StreamLine::GenStreamLine2(int i, int j, int L){
	origin = Center(i, j);
	//getVF(b);
	fwd[0] = RK2(origin,Ht, 0);
	if(fwd[0].coord[0] == origin.coord[0] && fwd[0].coord[1] == origin.coord[1]){
		for(int k = 0; k<L; k++){
			fwd[k].coord[0] = -1;
			fwd[k].coord[1] = -1;
...
Vector getVF2(Point pt, int d){
	Vector vec;
	Direction2D vf;
	if(pt.coord[0]>=0 && pt.coord[1]>=0 && pt.coord[0]<vectorfield2->_w && pt.coord[1]<vectorfield2->_h){
		//if(!t1){
			vf =  vectorfield2->get(pt.coord[0], pt.coord[1]);
		//}else  vf =  vectorfield2->get(pt.coord[0], pt.coord[1]);
		if(d==0){
			vec.coord[0] = vf.dx;
			vec.coord[1] = vf.dy;
