...
class PenroseQuasisamplerW : public Quasisampler {
	unsigned int val;
	int width; 
	int height;
...
public:
	PenroseQuasisamplerW(unsigned int _val, int _width, int _height, std::vector<unsigned int> &_imap) : 
		Quasisampler(_width, _height), val(_val), width(_width), height(_height), imap(_imap) {}

	unsigned int getImportanceAt(Point2D pt) { 
		int index = int(pt.x) + int(pt.y)*width;
		
		return val*(float(255-imap[index])/255.0f); 
	}
};

void PenroseSamplerW::complete()
{
	PenroseQuasisamplerW s((unsigned int) (9.1/(radius*radius)), width, height, imap);
	std::vector<Point2D> pts = s.getSamplingPoints();

	for (std::vector<Point2D>::iterator it=pts.begin(); it!=pts.end(); it++ ) {
		Vec2 pt( ((float) it->x)/width - 1.0f, ((float) it->y)/height - 1.0f);
		//Vec2 pt((float) it->x - width/2.f, (float) it->y - height/2.f);
